# =====================
# Downloaded from https://www.mikrobio.uni-kiel.de/de/ag-dagan/ressourcen
# 2021-01-23
...
Usage:
        mad infilename outfilename [-mnsptfgvh]

    Where the file contains tree(s) in NEWICK format.
    Rooted tree(s) will be written to 'filename.rooted.nwk',
...
# ------- short circut init - deferred imports
try:
    with open(fn) as x:
        nwkstr = x.read()
...
nnwk = len(mnwk)
if nnwk == 0:
    exit(err + "No NEWICK string found.")
if nnwk > 1 and not flags["M"] and not flags["N"]:
    exit(
        err
...
# ===== deal outputs
def madlog(s):
    global fh, outfile, gdbug_level, flags
    if s[0] == ">":
        pref = ""
    elif s[:5] == "Error":
        pref = "<<< "
...
def str2len(s):
    try:
        ...
        madlog(
            err + "Corrupt NEWICK format: invalid branch length (...'" + s + "'...)."
        )
    if not np.isfinite(x):
        madlog(
            err
            + "Cowardly refusing to root trees with infinite or undefined branch lengths (...'"
            + s
            + "'...)."
        )
    return x
...
def nwk2tree(nwkstr):
    ...
    if nc[0] != 1 or nwkstr[-1] != ";":
        madlog(
            err
            + "Corrupt NEWICK format - expecting exactly one string terminated with ';'."
        )
    if nc[3] != nc[4]:
        madlog(err + "Corrupt NEWICK format - unbalanced ().")
    notu = nc[1] + 1
    if notu < 3:
        madlog(err + "Cowardly refusing to root trees with less than 3 OTUs.")
    if nc[2] > 2 * notu - 2:
        madlog(err + "Corrupt NEWICK format - too many ':' | too few ','.")
    if nc[3] > notu - 1:
        madlog(err + "Corrupt NEWICK format - too many '()' | too few ','.")
    nnode = notu * 2 - 1
    rootnode = nnode - 1
    ...
    while s2[0] == "(":
        ...
        if len(c3) != 3:
            madlog(err + "Corrupt NEWICK format: () do not balance.")
        b2 = c3[1].split(",")
        if len(b2) < 2:
            madlog(
                err + "Corrupt NEWICK format: empty, singleton or unbalanced group ()."
            )
        elif len(b2) > 2:
        ...
        for i in [0, 1]:
            ...
            if b[0] != "@":
                ...
                if len(z) != 2:
                    madlog(
                        err
                        + "Corrupt NEWICK format: malformed 'node_label:branch_length' clouse (...'"
                        + b
                        + "'...)."
                    )
                if labs.count(z[0]) > 0:
                    madlog(
                        err
                        + "Cowardly refusing to root trees with duplicate OTU names ('"
                        + z[0]
                        + "')."
                    )
                b = "@{}@{}".format(iotu, b)
    ...
    if ntiny > 0 and not flags["T"]:
        madlog(
            "Warning: {} tiny branch lengths (<10^-6) were converted to 0. (Override with '-t'.)".format(
                ntiny
            )
        )
    ...
    if tlen == 0:
        madlog(err + "Cowardly refusing to root zero-lengthed trees.")
    slen = np.sort(blen)
    n = sum(np.diff(slen[slen > 0]) == 0)
    if n > 0:
        madlog(
            "Warning: Trees with repeating branch lengths are suspicious ({} repeating values).".format(
                n
            )
        )
    if len(badbsp) > 0:
        madlog(
            "Warning: Non-numeric or negative bootstrap values ignored. ({}).".format(
                badbsp
            )
        )
...
def pwdist_preproc():
    ...
    if ntipp > 0:
        madlog(
            "Warning: Squeezing tip polytomies ({} OTUs, {} redundant tips, {} effective OTUs).".format(
                notu, ntipp, enotu
            )
        )
    if enotu < 3:
        madlog(err + "Cowardly refusing to root trees with less than 3 effctive OTUs.")
    # ======= preproc in lists
    kij = [[]] * 4
    dij = [[]] * nnode
    for i in range(nnode):
...
def tree2nwk():
        ...
        if k < notu:
            ...
            if flags["G"]:
                if rlen[k] == 0:
                    ...
                    nwk[k] = "({}{}{}){}{}".format(
                        labs[k], nads[k], blens1[k], bads[k], blens2[k]
                    )
            else:
            ...
            if flags["G"]:
                if blen[k1] == 0:
                    ...
                    if rlen[k1] == 0:
                        ...
                        if k1 == k:
                            nwk[k] = "(({}){}{}){}{}".format(
                                nwk[k], nads[k], blens1[k1], bads[k1], blens2[k1]
                            )
                        else:
                            nwk[k] = "(({}){}{}){}{}".format(
                                nwk[k], nads[k], blens2[k1], bads[k1], blens1[k1]
                            )
    ...
    if abs(tol) > 0.0000001:
        ...
        madlog(bug + " tlen {} clen {} tol {}".format(tlen, clen, tol))
    # gdbug(2,newnwk,"\n----\n",nwk)
    return newnwk
...
def mad_output():
    global nodes, blen, trip, labs, outfile, rlen, rlen2, ad, nad
    # --------- find mads
    mad = min(ad)
    if mad < 0.001:
        madlog("Warning: MAD=={:.5g} is too good to be true.".format(mad))
    ...
    for r in range(nroots):
        ...
        rlen2[k1] = rlen2[i]
        rlen2[i] = 0
        if blen[k1] == 0 or blen[i] == 0:
            madlog("Warning: Root is polytomous.")
        # gdbug(1,nodes,blen,labs)
        # ------- make newick string
        rootstr = tree2nwk().replace(at, "@")
        s = "[MAD={:#5.3f}_AI={:#5.3f}_CCV={:#.3g}%_N={}/{}]".format(
            mad, ai, ccv[i] * 100, r + 1, nroots
        )
        madlog(">> " + s)
        if flags["S"]:
            rootstr += s
        if flags["F"] or flags["G"]:
            s = '[&AD={:#5.3f},ADS="MAD={:#5.3f}",STT="CCV={:#.3g}%"]:{:.3g})[&ADS="AI={:#5.3f}"]'.format(
                mad, mad, ccv[i] * 100, depth[i] * 0.1, ai
            )
    ...
    if flags["M"]:
        if nroots > 1:
            writeout(">> Tied root positions, {} rooted trees:".format(nroots))
        else:
            writeout(">> Rooted tree:")
    if flags["F"] or flags["G"]:
        rooted = (
            "#NEXUS\nbegin trees;\n"
            + rooted
...
for inwk in range(nnwk):
    if flags["M"] or flags["N"]:
        ...
        if flags["M"]:
            writeout(">Rooting of input tree #{}:".format(inwk + 1))
    else:
