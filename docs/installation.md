...
Although the installation and set up has been carefully thought through, using best-practice approaches, this is always a challenge. No amount of help documentation for a programme like this can help you in all situations. Some of the problems you may encounter will require investigation on your own.
...
The first two sections, git and conda/mamba are the most difficult but only have to be done once. This works well on macOS and Linux. Windows is not well tested but may run on Windows Subsystem for Linux.

## git

- The workflow is hosted on github.com/davelunt/NemaTree
...
Prepare the software environment using `mamba` and the `workflow/envs/environment.yaml` file. Move to the repository directory and try:

`mamba env create -f workflow/envs/environment.yaml`

This should install all the software as described in the environment.yaml file. At the command line you should make sure that this environment is activated with `conda activate nematree`
...
Most things have sensible defaults, but you must specify the name and location of your sample file to analyse. See [sequence help doc](docs/sequence_prep.md)
...
If you do not get lots of angry red error text then it will run, reporting what it is doing, until the prompt reappears and it is finished. For a small number of sequences this takes 1-2 minutes on a M2 MacBook Air from 2023.
